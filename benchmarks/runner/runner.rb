...
module Runner
  ...
  class Runner
    ...
    #
    def initialize(build_id:, runner_client:, report_client:, target:, runner:)
      @action = ''
      @stats = []
      @warmups = 0
      @repetitions = 0
    ...
    def run!
      ...
      @run_duration = ((Process.clock_gettime(Process::CLOCK_MONOTONIC)-run_start) * 1e+9 ).to_i
        ...
        payload = slice.map do |s|
          { index: {
              data: {
                ...
                labels: { client: 'elasticsearch-ruby' },
